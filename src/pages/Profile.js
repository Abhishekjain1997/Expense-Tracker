  ...
  const updateHandler = async (e) => {
    ...
    try {
      const res = await fetch(
        "https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyBjR0ypjDfdOXn_dk3wDjQEnYsoczmXlqs",
        {
          method: "POST",
          body: JSON.stringify({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(
          "https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyBjR0ypjDfdOXn_dk3wDjQEnYsoczmXlqs",
          {
            method: "POST",
            body: JSON.stringify({
              idToken: token,
